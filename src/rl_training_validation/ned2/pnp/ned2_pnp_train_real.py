#!/usr/bin/env python3
"""
Train an SB3 policy on the Ned2 *real* Pick-and-Place task.

PnP differs from push in two ways:
  * 7-DOF action (6 arm joints + 1 gripper scalar). The gripper scalar
    is midpoint-discretized into the binary niryo tools-commander
    "open" / "close" action (no continuous position control).
...
See ned2_push_train_real.py for the cube-tracking + safety gating
...
ENV_STD = "NED2PnPReal-v0"
ENV_GOAL = "NED2PnPGoalReal-v0"
...
def main() -> int:
    ...
    if args.goal:
        cfg = CFG_GOAL
        save_path = "/models/real/td3_goal/ned2/pnp/"
        log_path = "/logs/real/td3_goal/ned2/pnp/"
        ModelCls = TD3_GOAL
    else:
        cfg = CFG_STD
        save_path = "/models/real/td3/ned2/pnp/"
        log_path = "/logs/real/td3/ned2/pnp/"
        ModelCls = TD3
