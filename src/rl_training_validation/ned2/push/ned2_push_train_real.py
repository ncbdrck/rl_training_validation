...
prints a throttled warning — usable for dry runs but obviously not for
training. Wire up ``rl_envs_cube_tracker`` (AprilTag) or any equivalent
detector before training for real.

For a one-line AprilTag bringup, pass ``--cube-tracker auto`` — the env
will spawn ``rl_envs_cube_tracker/<camera>.launch`` under its managed
process registry so ``env.close`` reaps it.
"""
