#!/usr/bin/env python3
"""
Validate a trained SB3 policy on the RX200 *real* Push task.

Loads a previously-trained policy (TD3 or TD3+HER for --goal) and
runs the requested number of episodes on the real robot.

See rx200_push_train_real.py for the safety gating + cube tracking
...
def main() -> int:
    ...
    if args.goal:
        base = "/models/real/td3_goal/rx200/push/"
        ModelCls = TD3_GOAL
        cfg = "rx200_push_td3_goal.yaml"
    else:
        base = "/models/real/td3/rx200/push/"
        ModelCls = TD3
        cfg = "rx200_push_td3.yaml"
    ...
    if not os.path.exists(abs_model_path + ".zip"):
        raise SystemExit(
            ...
            "or run rx200_push_train_real.py first."
        )
