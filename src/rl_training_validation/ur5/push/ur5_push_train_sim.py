#!/usr/bin/env python3
"""
Train an SB3 policy on the UR5 sim Push task.

Standard env id:  ``UniROS-UR5PushSim-v0``
Goal env id:      ``UniROS-UR5PushGoalSim-v0``

Requires Gazebo + roscore to already be running with the
rl_environments-owned UR5 workspace world (provides the workbench the
cube spawns onto). Bring it up like:
...
The env class then launches the UR5 MoveIt stack itself. Without that
world, the cube spawned by ``UR5RobotEnv.spawn_cube_in_gazebo`` falls
to the ground plane and the task geometry is wrong.
...
def main() -> int:
    ...
    if args.goal:
        cfg = CFG_GOAL
        save_path = "/models/sim/td3_goal/ur5/push/"
        log_path  = "/logs/sim/td3_goal/ur5/push/"
        ModelCls = TD3_GOAL
    else:
        cfg = CFG_STD
        save_path = "/models/sim/td3/ur5/push/"
        log_path  = "/logs/sim/td3/ur5/push/"
        ModelCls = TD3
