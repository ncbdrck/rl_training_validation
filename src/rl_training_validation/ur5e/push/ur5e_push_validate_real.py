#!/usr/bin/env python3
"""
Validate a trained SB3 policy on the UR5e *real* Push task.

Loads a previously-trained policy (TD3 or TD3+HER for --goal) and
runs the requested number of episodes on the real robot.

See ur5e_push_train_real.py for the safety gating + cube tracking
...
from rl_training_validation.utils.env_safety import (
    ...
    apply_cube_tracker_kwargs, apply_goal_pose_kwargs,
...
ENV_STD = "UR5ePushReal-v0"
ENV_GOAL = "UR5ePushGoalReal-v0"
...
def main() -> int:
    ...
    # Resolve + verify the model file exists BEFORE bringing up the real
    # robot. SB3 saves as <path>.zip; reach validate scripts use the same
    # base + tag scheme.
    pkg_path = "rl_training_validation"
    if args.goal:
        base = "/models/real/td3_goal/ur5e/push/"
        ModelCls = TD3_GOAL
        cfg = "ur5e_push_td3_goal.yaml"
    else:
        base = "/models/real/td3/ur5e/push/"
        ModelCls = TD3
        cfg = "ur5e_push_td3.yaml"
    ...
    if not os.path.exists(abs_model_path + ".zip"):
        raise SystemExit(
            ...
            "or run ur5e_push_train_real.py first."
        )
    ...
    # fresh untrained policy. Required: a plain ModelCls(env, ...) call
    # would create a new model and predict() would run a random policy.
