...
Mirrors ``ur5e_reach_train_sim.py`` for env construction; loads a
saved model and rolls it out for ``--episodes`` episodes, logging
success rate, truncations, and sensor timeouts from ``info``.

Requires Gazebo + roscore + the UR5 workbench world to be running
(see the train script docstring for the launch command).
"""
...
ENV_STD  = "UR5eReachSim-v0"
ENV_GOAL = "UR5eReachGoalSim-v0"
...
def main() -> int:
    ...
    if args.goal:
        ...
        base = "/models/sim/td3_goal/ur5e/reach/" if args.algo == "td3" else "/models/sim/sac_goal/ur5e/reach/"
        ModelCls = TD3_GOAL if args.algo == "td3" else SAC_GOAL
    else:
        cfg = CFG_STD_TD3 if args.algo == "td3" else CFG_STD_SAC
        base = "/models/sim/td3/ur5e/reach/" if args.algo == "td3" else "/models/sim/sac/ur5e/reach/"
