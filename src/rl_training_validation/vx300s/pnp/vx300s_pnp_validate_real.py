#!/usr/bin/env python3
"""
Validate a trained SB3 policy on the VX300S *real* Pick-and-Place task.

Loads a previously-trained policy (TD3 or TD3+HER for --goal) and
runs the requested number of episodes on the real robot.

See vx300s_pnp_train_real.py for the safety gating, cube-tracking,
...
def main() -> int:
    ...
    if args.goal:
        base = "/models/real/td3_goal/vx300s/pnp/"
        ModelCls = TD3_GOAL
        cfg = "vx300s_pnp_td3_goal.yaml"
    else:
    ...
    if not os.path.exists(abs_model_path + ".zip"):
        raise SystemExit(
            ...
            "or run vx300s_pnp_train_real.py first."
        )
