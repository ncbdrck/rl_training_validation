...
def main() -> int:
    ...
    if args.goal:
        if args.algo == "td3":
            config_file_name = "vx300s_reacher_td3_goal.yaml"
            save_path = "/models/real/td3_goal/vx300s/reach/"
            log_path = "/logs/real/td3_goal/vx300s/reach/"
            model = TD3_GOAL(env, save_path, log_path, model_pkg_path=pkg_path,
                             config_file_pkg=pkg_path, config_filename=config_file_name)
        else:
            config_file_name = "vx300s_reacher_sac_goal.yaml"
            save_path = "/models/real/sac_goal/vx300s/reach/"
            log_path = "/logs/real/sac_goal/vx300s/reach/"
            model = SAC_GOAL(env, save_path, log_path, model_pkg_path=pkg_path,
                             config_file_pkg=pkg_path, config_filename=config_file_name)
    else:
        if args.algo == "td3":
            config_file_name = "vx300s_reacher_td3.yaml"
            save_path = "/models/real/td3/vx300s/reach/"
            log_path = "/logs/real/td3/vx300s/reach/"
            model = TD3(env, save_path, log_path, model_pkg_path=pkg_path,
                        config_file_pkg=pkg_path, config_filename=config_file_name)
        else:
            config_file_name = "vx300s_reacher_sac.yaml"
            save_path = "/models/real/sac/vx300s/reach/"
            log_path = "/logs/real/sac/vx300s/reach/"
